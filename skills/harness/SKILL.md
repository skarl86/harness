...
모든 아티팩트는 프로젝트 루트의 `.harness/` 하위에 **요청별 영어 슬러그 폴더**를 만들어 저장한다.
이로써 서로 다른 요청의 아티팩트가 섞이지 않는다.

```
.harness/
└── {slug}/                  # 요청에 맞는 영어 슬러그 폴더 (kebab-case)
...
> 이하 문서에서 `{BASE}`는 `.harness/{slug}/`를 의미한다.

## 실행 흐름

### 사전 준비

1. **슬러그 생성**: 사용자 요청 `$ARGUMENTS`의 핵심을 요약해 **영어 kebab-case 슬러그**를 만든다.
   - 소문자, 영숫자와 `-`만 사용, 최대 50자
   - 예: "로그인 기능 추가" → `add-login-feature`, "상품 검색 API" → `product-search-api`
   - 한국어 요청이라도 슬러그는 반드시 영어로 작성한다
2. `.harness/` 디렉토리가 없으면 생성한다.
3. `.harness/{slug}/` 디렉토리 존재 여부를 확인한다.
4. **이미 존재하면 → Resume 감지**를 수행한다:
   a. 아래 순서로 artifact를 확인하여 마지막 완료 단계를 판별한다:
      - `{BASE}05-evaluate.md` 존재 → 파이프라인 완료 상태
      - `{BASE}04-generate/` 존재 → Step 4 진행 중 또는 완료
      - `{BASE}03-plan/` 존재 → Step 3 완료
      - `{BASE}02-context.md` 존재 → Step 2 완료
      - `{BASE}01-clarify.md` 존재 → Step 1 완료
      - `{BASE}00-request.md`만 존재 → 시작 전
   b. Step 4 진행 중인 경우, `{BASE}04-generate/task-*.md` 파일들을 읽어 각 Task의 완료 상태를 파악한다.
   c. 사용자에게 감지된 상태와 **슬러그 폴더 경로**를 보고하고, 어디서부터 재개할지 확인한다:
      - "`.harness/{slug}/`에서 Step N부터 재개합니다. 진행할까요?"
      - 사용자가 처음부터 다시 하길 원하면 기존 `.harness/{slug}/`를 정리한다.
      - 슬러그가 충돌하지만 내용이 다른 새 요청이라면, 사용자에게 확인 후 슬러그에 접미사(`-v2` 등)를 붙여 새 폴더를 만든다.
5. **존재하지 않으면** → `.harness/{slug}/` 디렉토리를 새로 생성한다.
6. 사용자의 요청 `$ARGUMENTS`를 `{BASE}00-request.md`에 원문 그대로 저장한다 (resume 시에는 기존 파일 유지).

> 이후 모든 단계에서 artifact 경로는 `{BASE}` 기준으로 해석한다. Agent 프롬프트에 경로를 전달할 때는 `{BASE}`를 **실제 경로 문자열**(`.harness/{slug}/`)로 치환해서 전달한다.
...
3. 사용자의 피드백을 `{BASE}01-clarify.md`에 `## 사용자 피드백` 섹션으로 추가한다.
...
`{BASE}03-plan/` 의 Phase YAML 파일들을 읽고, 각 Task를 sub-agent로 실행한다.
...
**A. 사전 점검 (실행 전)**
1. `{BASE}04-generate/task-{id}.md`가 이미 존재하고 `status: success`이면 → **건너뛴다**.
...
Agent 반환 결과를 `{BASE}04-generate/task-{id}.md`에 기록한다:
...
모든 Task 완료 후 (또는 중단 시), `{BASE}04-generate/summary.md`를 작성한다:
...
2. 필요시 `{BASE}05-evaluate.md`의 상세 내용을 안내한다.
...
2. **Artifact 위치**: `.harness/{slug}/` 디렉토리 내 파일 목록
